    ...
    //            let rect = self.boundingRect(forGlyphRange: glyphsToShow, in: self.textContainers.first!);
            ...
            textStorage!.enumerateAttribute(.paragraphStyle, in: charRange, options: [], using: { (value, range, pth) in
                ...
                if paragraph.tailIndent != 0 {
                    ...
                    let path = UIBezierPath(rect: CGRect(origin: rect.origin, size: CGSize(width: 2, height: rect.height)));
                    path.fill();
                    ...
                    let path = UIBezierPath(rect: CGRect(x: (rect.origin.x > paragraph.firstLineHeadIndent) ? 1 : rect.origin.x, y: rect.origin.y, width: 2, height: rect.height));
                    path.fill();
                }
            })
        }
